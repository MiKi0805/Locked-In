...
# local hp
@onready var health_counter : Health_system = $"Health system"
var hp : int
...
func get_current_hp():
	hp = health_counter.health_points
	hp_after = hp
...
func change_scale():
	if hp_before != hp_after:
		...
		if hp_after < hp_before: # hp decrease
			target_scale -= leap
			tween_scale.tween_property(self, 'scale', target_scale, 0.1)
...
func focused(requester : Node2D):
	...
	if focused_enemy:
		focus_to = true
		$Combine/CollisionPolygon2D.disabled = true
